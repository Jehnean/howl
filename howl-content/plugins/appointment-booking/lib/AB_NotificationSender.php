...
abstract class AB_NotificationSender
{
    ...
    private static function _prepareData( AB_CustomerAppointment $ca )
    {
        ...
        $codes->set( 'category_name',     AB_Utils::getTranslatedString( 'category_' . $service->get( 'category_id' ), $service->getCategoryName() ) );
        $codes->set( 'client_name',       $customer->get( 'name' ) );
        $codes->set( 'client_phone',      $customer->get( 'phone' ) );
        $codes->set( 'client_email',      $customer->get( 'email' ) );
        ...
        $codes->set( 'service_name',      AB_Utils::getTranslatedString( 'service_' . $appointment->get( 'service_id' ), $service->get( 'title' ) ) );
        $codes->set( 'service_price',     $price );
        $codes->set( 'staff_name',        AB_Utils::getTranslatedString( 'staff_' . $appointment->get( 'staff_id' ), $staff->get( 'full_name' ) ) );
     ...
     * @return bool
     */
    private static function _send( AB_Notification $notification, AB_NotificationCodes $codes, $mail_to, $phone = '', $extra = array(), $language_code = null )
    {
        $result  = false;
        $message_template = AB_Utils::getTranslatedString( $notification->get( 'gateway' ) . '_' . $notification->get( 'type' ), $notification->get( 'message' ), $language_code );
        $message = $codes->replace( $message_template, $notification->get( 'gateway' ) );
        if ( $notification->get( 'gateway' ) == 'email' ) {
            // Send email to recipient.
            $subject = $codes->replace( AB_Utils::getTranslatedString( $notification->get( 'gateway' ) . '_' . $notification->get( 'type' ) . '_subject', $notification->get( 'subject' ), $language_code ) );
