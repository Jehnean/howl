...
class AB_Captcha
{
    ...
    static function init( $form_id, $config = array() )
    {
        ...
        $_SESSION['AB_CAPTCHA'][ $form_id ]['config'] = serialize( $captcha_config );
    }
    ...
    {
        $captcha_config = unserialize( $_SESSION['AB_CAPTCHA'][ $form_id ]['config'] );
    ...
    {
        if ( isset( $_SESSION['AB_CAPTCHA'][ $form_id ] ) ) {
            $captcha_config = unserialize( $_SESSION['AB_CAPTCHA'][ $form_id ]['config'] );
            if ( ! $captcha_config ) {
                return true;
            } else {
