...
class Astoundify_Updater_Claims {
	...
	public function license_field( $fields ) {
        if ( ! isset( $fields[ 'wpjmcl_settings' ] ) ) {
            $fields['wpjmcl_settings'] = array(
                __( 'Claim Listing', 'wp-job-manager-claim-listing' ),
                array()
            );
        }

		$fields[ 'wpjmcl_settings' ][1][] = array(
			'name'			=> $this->slug,
			'type'          => 'license',
			'std'			=> '',
			'placeholder'	=> '',
			'label'			=> __( 'License Key', 'wp-job-manager-claim-listing' ),
			'desc'			=> __( 'Enter the license key you received with your purchase receipt.', 'wp-job-manager-claim-listing' ),
			'attributes'	=> array()
		);
	...
	public function output_license_field( $option, $attributes, $value, $placeholder ) {
		...
		?>
			...
			<?php if( $status !== false && $status == 'valid' ) { ?>
				...
				?>
				<a href="<?php echo esc_url( add_query_arg( $args, wp_nonce_url( admin_url( '/edit.php?post_type=job_listing&page=job-manager-settings' ), 'astoundify_license_deactivate' ) ) ); ?>" class="button-secondary"><?php _e( 'Deactivate', 'wp-job-manager-claim-listing' ); ?></a>
			<?php  } ?>
			</p>
		<?php
	}
