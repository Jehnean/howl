...
class Listify_WP_Job_Manager_Map_Template extends listify_Integration {
	...
	private function get_average_radius() {
		$default = isset( $_GET[ 'search_radius' ] ) ? absint( $_GET[ 'search_radius' ] ) : listify_theme_mod( 'map-behavior-search-default' );
	...
	public function enqueue_scripts( $force = false ) {
        ...
        if ( listify_theme_mod( 'map-behavior-api-key' ) ) {
            $args[ 'key' ] = listify_theme_mod( 'map-behavior-api-key' );
        }
		...
		wp_enqueue_script( 'google-maps', esc_url_raw( add_query_arg( $args, $base ) ), array(), null, false );
